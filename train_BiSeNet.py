...
def main():
    ...
    if cfg.TRAIN.STAGE=='all':
        net = BiSeNet(num_classes =5, context_path='resnet18')
        ...
        net = BiSeNet(num_classes=5, context_path='resnet101')
    if len(cfg.TRAIN.GPU_ID)>1:
...
def train(train_loader, net, criterion, optimizer, epoch):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        optimizer.zero_grad()
        outputs , outputs_sup1,outputs_sup2= net(inputs)
        '''
        loss1 = criterion(outputs.squeeze(1), labels)
        loss2 = criterion(outputs_sup1.squeeze(1), labels)
        loss3 = criterion(outputs_sup2.squeeze(1), labels)
        '''
        loss1 = criterion(outputs, labels)
        loss2 = criterion(outputs_sup1, labels)
        loss3 = criterion(outputs_sup2, labels)
